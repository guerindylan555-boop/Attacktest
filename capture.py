...
OUTPUT_FILE = "CAPTURED_API.txt"
OUTPUT_JSON = "CAPTURED_API.json"
...
def on_message(message, data):
    ...
    if msg_type == 'api_coroutine_unlock':
        ...
        write_block(f"[{timestamp}] [COROUTINE UNLOCK]", [
    ...
    if msg_type == 'api_coroutine_lock':
        ...
        write_block(f"[{timestamp}] [COROUTINE LOCK]", [
            f"Authorization: {auth}",
            f"Pass ID: {pass_id}",
            f"Temporary: {temporary}"
        ])
...
def main():
    print('=' * 100)
    print('  MAYNDRIVE TRAFFIC CAPTURE - FULL STACK')
    print('=' * 100)
    print('')
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        f.write('=== MAYNDRIVE TRAFFIC CAPTURE ===\n')
    ...
    try:
        ...
        print('')
        print('=' * 100)
        print('  READY TO CAPTURE (HTTP + SOCKET.IO + UDP)')
        print('=' * 100)
        print('')
        ...
        print('    5. Watch this console for coroutine, HTTP, Socket.io, and UDP logs')
        print('')
        print('  Notes:')
